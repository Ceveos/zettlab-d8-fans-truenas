...
/*
 * MMIO register base address provided by Zettlab (support@zettlab.com).
 * This is intentionally non-page-aligned — the hardware exposes fan
 * control registers at byte-granularity offsets within a memory-mapped
 * region. ioremap handles the internal page alignment transparently.
 */
#define FAN_BASE_ADDR 0xFE0B0456
#define FAN_REG_SIZE  0x10

/* Register offsets from FAN_BASE_ADDR */
...
#define PWM_MAX_HW 0xB7  /* 183 decimal — maps to 100% fan speed */
                         ...
                         u32 attr, int channel, long val)
{
    ...
    case hwmon_pwm_enable:
        ...
        case 2: /* auto (fan3 only) */
            ...
            /* auto mode: PWM = 0 hands control back to BIOS/EC */
...
static int zettlab_probe(struct platform_device *pdev)
{
    ...
    /* Disk fans default to manual control (driven by control_hdd_fans.sh) */
    data->pwm_enable[0] = 1;
    data->pwm_enable[1] = 1;
    /* CPU fan defaults to auto — BIOS/EC retains control unless overridden */
    data->pwm_enable[2] = 2;

    if (!devm_request_mem_region(&pdev->dev, FAN_BASE_ADDR, FAN_REG_SIZE, DRIVER_NAME))
        return -EBUSY;

    data->base = devm_ioremap(&pdev->dev, FAN_BASE_ADDR, FAN_REG_SIZE);
    if (!data->base)
        return -ENOMEM;

    /* Ensure CPU fan is in auto mode (PWM=0 hands control to BIOS/EC) */
    zettlab_write_pwm(data, FAN3_PWM_OFFSET, 0);
